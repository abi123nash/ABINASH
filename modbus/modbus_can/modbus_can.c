...
void start_slave() {
    ...
    while (1) {
	    ...
	    printf("Received query:\n");
            ...
            uint32_t starting_address = (query[8] << 8) | query[9];
            uint16_t quantity = (query[10] << 8) | query[11];

           for(z=0;z < register_count ; z++)
           {
	    ...
	    frame_send.can_dlc = 8;
            ...
            for(y=0; y<8; y++)
	    {
	       frame_send.data[y] = reg_data->data[y];
	    }	    
            ...
            if (frame.can_dlc >= 8) 
	    {
		  ...
		  printf("Mapped CAN data to Modbus registers: 0x%X, 0x%X, 0x%X, 0x%X\n", 
                        mb_mapping->tab_registers[0], mb_mapping->tab_registers[1],mb_mapping->tab_registers[2], mb_mapping->tab_registers[3]);
            }
	        ...
	        query[9] = 0;
                ...
                if (reply_status == -1) {
                  ...
                  return; // Or any appropriate error code
               }
