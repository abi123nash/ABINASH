...
/*
 * Function to initialize the registers dynamically
 *
 * This function allocates memory for the register array, assigns valid register
 * addresses, and initializes their data with placeholder values for demonstration.
 */
void init_registers(size_t count) {
    ...
    {
        registers[i].address = valid_register_addresses[i];  /* Assign register address */
        for (int j = 0; j < 8; j++)
        {
 ...
 *
 * This function searches the register array for a register with the specified address.
 * If found, it returns a pointer to the corresponding register data.
 * If not found, it returns NULL.
 *
 * @param address The register address to search for.
 *
 * @return A pointer to the register data if found, otherwise NULL.
 */
RegisterData* get_register_data(uint32_t address) 
{
    /*
     * Loop through the register array to find the register with the matching address
     */
    for (int i = 0; i < register_count; i++) 
    {
        if (registers[i].address == address) 
        {
            return &registers[i];  /* Return the pointer to the register data */
        }
    }

    /*
     * Return NULL if the address is not found in the register array
     */
    return NULL;
}
 ...
 * It uses perror to output the error message followed by the provided message.
 *
 ...
 * Setup CAN interface
 *
 * This function configures the CAN interface by first bringing the interface down, then setting it up with the
 * specified bitrate using the `ip` command. If any step fails, an error message is printed, and the program exits.
 *
 * @param interface The name of the CAN interface (e.g., "can0").
 * @param bitrate The bitrate for the CAN interface (e.g., 500000).
 */
void setup_can_interface(const char *interface, int bitrate)
{
    ...
    /*
     * Ensure the CAN interface is fully brought up
     */
 ...
 * Send a CAN message
 *
 * This function sends a CAN frame using the provided socket file descriptor. If the sending operation fails,
 * an error message is printed, and the program exits.
 *
 * @param socket_fd The socket file descriptor for the CAN interface.
...
void send_can_message(int socket_fd, struct can_frame *frame)
{
    ...
    /*
     * Print the details of the sent CAN frame
     */
 ...
 *
 * This function receives a CAN frame using the provided socket file descriptor. If the receiving operation fails,
 * an error message is printed, and the program exits.
 *
 * @param socket_fd The socket file descriptor for the CAN interface.
...
void receive_can_message(int socket_fd, struct can_frame *frame)
{
    ...
    /*
     * Print the details of the received CAN frame
     */
...
/*
 * Function to start Modbus slave and handle requests
 */

void start_slave()
{
    /*
     * Define Modbus context and CAN frame variables
     */
    modbus_t *mb;
    ...
    /*
     * Main loop to receive Modbus requests
     */
    while (1)
    {
        ...
        if (rc > 0)
        {
            ...
            /*
             * Adjust starting address if necessary
             */
            ...
            if (reg_data != NULL)
            {
                ...
                /*
                 * Send the response to the Modbus master
                 */
 ...
 *
 * This function releases the memory allocated for the `registers` array.
 * After freeing the memory, it ensures the pointer is set to NULL to avoid
 * dangling pointers.
 */
void free_registers()
{
    ...
    {
        /*
         * Free the memory allocated for the registers
         */
        free(registers);
        registers = NULL;
        printf("Registers memory successfully freed.\n");
    }
    else
    {
        /*
         * Handle the case where registers are already NULL
         */
        printf("Registers memory is already freed or was never allocated.\n");
    }
}
...
{
    /*
     * Calculate the number of valid registers from the array
     */
    register_count = sizeof(valid_register_addresses) / sizeof(valid_register_addresses[0]);

    /*
     * Initialize registers based on the calculated register count
     */
    init_registers(register_count);

    /*
     * Start the slave process (implementation of this function assumed)
     */
    start_slave();

    /*
     * Free the allocated memory for registers before exiting
     */
    free_registers();
