...
#define TIMEOUT_SEC    2  // Timeout for select in seconds
...
void init_registers(size_t count) {
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            registers[i].data[j] = (uint16_t)(0x10 + i);
        }
    }
}
...
/*
 * Receive a CAN message with select() and timeout handling
 */
int receive_can_message_with_timeout(int socket_fd, struct can_frame *frame, int timeout_sec)
{
    fd_set read_fds;
...
void start_slave()
{
    ...
    mb_mapping = modbus_mapping_new(0, 0, 1000, 0);  
    if (mb_mapping == NULL)
    {
    ...
    while (1)
    {
        ...
        if (rc > 0)
        {
            ...
            if (reg_data != NULL)
            {
                ...
                if (receive_can_message_with_timeout(socket_fd, &frame, TIMEOUT_SEC) == 0)
                {
                    // Process CAN data
                    for (int i = 0; i < 4; i++)
                    {
