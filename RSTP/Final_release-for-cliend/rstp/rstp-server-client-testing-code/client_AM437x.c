 ...
 *  @brief 
 *
 *  This source file implements a UDP client that receives broadcast messages 
 *  from a server and returns the server's IP address to send port status.
 *  Additionally, the client controls LEDs based on the STP (Spanning Tree Protocol) status of the br0 bridge.
 *  The LED states reflect the status of the bridge configuration, with a green LED indicating a valid configuration
 *  and a red LED indicating issues with the configuration.
 *
 *  @author Abinash
 *
 *  @bug No known bugs.
...
/** @brief send_port_status
 *
 *  This function sends the status of two ports over UDP to a specified server.
 *
 *  @param sender_ip : The IP address of the server to send the status to.
 *  @param status1   : The status of the first port to be sent.
 *  @param status2   : The status of the second port to be sent.
 *
...
/** @brief monitor_ports
 *
 *  This function monitors the status of two network ports, checking if their states have changed.
 *  If the states change, it sends the updated status to a specified server. This function runs in a separate thread.
 *
 *  @param arg : Pointer to any argument passed to the thread (not used in this function).
 *
 *  @return void* : Returns NULL when the function completes (infinite loop).
 */
void *monitor_ports(void *arg) 
{
    ...
    while (1)
    {
        ...
        /*
         * Retry sending updated status if there was a failure in the previous attempt
         */
        if (flag == -1)
        {
            flag = send_port_status(sender_ip, &curr_status1, &curr_status2);
...
/** @brief udp_client
 *
 *  This function creates a UDP client that listens for incoming messages on a specified port.
 *  It uses the `select` function to monitor the socket for any incoming data with a timeout.
 *  If a message is received, it prints the message and updates the server connection status.
 *  It also handles timeouts and errors.
 *
...
void udp_client() 
{
    ...
    while (1)
    {
        ...
        if (activity == 0)
        {
            ...
            server_connect = 0;  // Update the flag if a message is not received
        }
        else if (activity < 0)
        {
            perror("Select error");
        ...
        else
        {
            ...
            {
		 write_gpio_value(PINS[2], 0);  // Green LED (Off)    
		     ...
		     write_gpio_value(PINS[2], 1);  // Green LED (Off)
		     sleep(1);
                    /*
...
/** @brief main
 *
 *  This function initializes and runs the program by creating two threads:
 *  - One for monitoring ports
 *  - One for controlling LEDs based on the RSTP status.
 *  It also runs the UDP client in the main thread.
 *
 *  @return int : Return status (EXIT_SUCCESS or EXIT_FAILURE)
 */
int main() {
