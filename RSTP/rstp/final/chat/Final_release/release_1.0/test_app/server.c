 ...
 * ----------------------------
 * Retrieves the broadcast address of an active, non-loopback network interface.
 * The function iterates through the list of network interfaces and identifies
 * the first interface with a valid broadcast address that is up and running.
 *
 * Parameters:
 *   broadcast_ip - A pointer to a character array where the broadcast address
 *                  will be stored.
 *
 * Returns:
 *   0 on success if a valid broadcast address is found.
 *  -1 on failure or if no suitable broadcast address is identified.
 */

int get_broadcast_address(char *broadcast_ip) {
    ...
    for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
        /* 
         * Check if the interface has a valid address and is IPv4 
         */
        if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET) {
            /*
             * Ensure the broadcast address is not null 
             */
            ...
            if (strcmp(ifa->ifa_name, "lo") != 0 &&
                ...
                /*
                 * Copy the broadcast address to the provided buffer 
                 */
                strcpy(broadcast_ip, inet_ntoa(sin->sin_addr));
                found = 1;
                break;
            }
        }
...
int main()
{
    ...
    char BROADCAST_Network[INET_ADDRSTRLEN];

    /*
     * Get the broadcast address dynamically for the first active network interface
     */
    if (get_broadcast_address(BROADCAST_Network) != 0) {
        printf("No broadcast address found.\n");
        return -1;
    }

    printf("Broadcast address found: %s\n", BROADCAST_Network);
    ...
    broadcast_addr.sin_addr.s_addr = inet_addr(BROADCAST_Network);
